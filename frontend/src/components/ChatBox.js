...
const ChatItem = ({ message, user }) => {
  return (
    <div className={`chat__item ${getSenderFull(user, message)}`}>
...
const ChatBox = () => {
  ...
  const fetchMyAllMessage = async () => {
    const { data } = await axios.post(
      "http://localhost:5000/allmessage",
  ...
  const { data: MyMessage } = useQuery(
    ["mychatWithUser", selectedChat._id],
    fetchMyAllMessage,
    {
      onSuccess: (data) => {
        console.log(data);
  ...
  const sendingMessageHandler = (e) => {
    if (e.keyCode === 13) {
      sendMessagemutation.mutate({
        chatid: selectedChat._id,
        to: getSenderId(user, selectedChat.participants),
        text: inputRef.current.value,
      });
    }
  };
  ...
  const makeMessageSeen = async (data) => {
    await axios.post(
      "http://localhost:5000/seethemessage",
      {
        chatid: data.chatid,
      },
      {
...
const getMessageToSpecific=(data)=>{
    ...
    console.log("hello inside getmessage")
...
if(socket){
  socket.on("getMessageToSpecific",getMessageToSpecific);
    
}
   
   

return () => socket.off('getMessageToSpecific', getMessageToSpecific);
  }, [socket,selectedChat]);
  ...
  return (
    <>
      {Object.keys(selectedChat).length !== 0 ? (
        <div className="main__chatcontent">
          <div className="content__header">
            ...
            <div className="headerIcon">
              <div className="first">
                ...
                <Link to="/insta/videocall">
                  <div>
                    {" "}
                    <VideoCamera size={25} weight="thin" />
                  </div>
                </Link>
                <div>
                  {" "}
                  <MagnifyingGlass size={25} weight="thin" />
                </div>
              </div>
              <div>
          ...
          <div className="content__body">
            <div className="chat__items">
              {MyMessage &&
                MyMessage.messages.map((itm, index) => {
                  return (
                    <ChatItem
        ...
        <div>
          <div className="main__chatcontent">please selectt the chat</div>
        </div>
      )}
    </>
  );
